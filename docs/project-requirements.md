...
- 左侧菜单栏展示 `开启新对话` 入口。
- `开启新对话` 图标使用写作/编辑语义图标，不使用简单加号。
- 点击 `开启新对话` 立即创建新的空会话并切换过去。
- 一级菜单仍为 `AI Chat`。
- `AI Chat` 展开后展示最近会话列表。
...
- 输入框下方展示两个 Prompts 提示：
  - `我要智能分柜`
  - `计划出货批次单号：\nSku信息：\n柜子：\n策略：同仓优先、重货先装并放底层`
- Prompts 提示项背景透明，只保留边框。
- Prompts 提示项文字前展示轻量 Unicode 图标。
- Prompts 提示项需要 hover 效果，包括边框色、淡背景、阴影或轻微位移反馈。
- 装柜信息模板 Prompt 在桌面宽度优先单行展示，点击填入输入框时仍保留多行模板。
